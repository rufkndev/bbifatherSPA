...
import { courseStructure, subjectsData, getSubjectById, calculateFullCoursePrice, calculateSelectedWorksPrice, SubjectData, getCourseById, getSemesterSubjects, getSemesterInfo } from '../data/subjects';
...
const CreateOrderPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    customSubject: '',
    customWork: '',
    customWorks: [] as { title: string; description: string }[],
    title: '',
    description: '',
    inputData: '',
    variantInfo: '',
  ...
  const canProceedToNextStep = (): boolean => {
    switch (activeStep) {
      case 0: // Выбор курса
        return formData.courseNumber > 0 || formData.isCustom;
      case 1: // Выбор семестра
        if (formData.isCustom) return true;
        return formData.semesterNumber > 0;
      ...
      case 3: // Выбор работ
        const semesterInfo = getSemesterInfo(formData.courseNumber, formData.semesterNumber);
        const selectedSubject = getSubjectById(formData.subjectId);
        
        if (formData.isCustom || semesterInfo?.allowCustomInput || selectedSubject?.customWorks) {
          return formData.customWorks.length > 0 && formData.customWorks.every(w => w.title.trim() !== '');
        }
